...
public class Equipment : Item
{
    ...
    #region 장착아이템 생성 메서드
    //생성자가 호출되기 이전에 메서드가 생성되어야 하므로 static 메서드로 구현
    //무기 생성 메서드
    public static Equipment CreateWeapon(string weaponType)
    {
        ...
        {
            case "목검":
                return new Equipment("목검", "기본 무기", 100, EquipmentSlot.Weapon, 5, 0);
            case "철검":
                return new Equipment("철검", "강력한 무기", 500, EquipmentSlot.Weapon, 15, 0);
            case "전설검":
                return new Equipment("전설검", "최고급 무기", 1000, EquipmentSlot.Weapon, 25, 0);
            default:
                return null;
        }
    }
